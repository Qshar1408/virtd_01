...
Для решения данной задачи подойдёт ПО Microsoft Hyper-V. Хорошая интеграция и поддержка как Windows, так и Linux виртуальных машин. 

2. Требуется наиболее производительное бесплатное open source-решение для виртуализации небольшой (20-30 серверов) инфраструктуры на базе Linux и Windows виртуальных машин.

Для решения данной задачи подойдёт Proxmox VE. 
Proxmox:
- бесплатная open source платформа;
- поддерживает как KVM для полной виртуализации, так и LXC для контейнеризации;
- позволяет эффективно использовать ресурсы и поддерживать как Linux, так и Windows виртуальные машины.
  
3. Необходимо бесплатное, максимально совместимое и производительное решение для виртуализации Windows-инфраструктуры.

Для решения данной задачи подойдёт KVM. Не смотря на преобладающее влияние на Linux-машинах, он поддерживает работу и на Windows.

4. Необходимо рабочее окружение для тестирования программного продукта на нескольких дистрибутивах Linux.

Для решения данной задачи подойдёт VirtualBox. Он простой в использовании, бесплатный и кросс-платформенный. 
...
1.Сложность управления (множество платформ и инструментов для управления)

2.Интеграция и совместимость (различные системы виртуализации, разные форматы данных и API, сложность миграции и резервирования)

3.Увеличение затрат (чем больше различных платформ, тем больше издержки)

4.Безопасность (чем больше систем, тем сложнее подготавливать всё к "защищенному виду")
...
1. Использовать мультиплатформенные решения для управления и мониторинга;

2. Выработать единые методывзаимодействия, которые применимы ко всем используемым платформам;

3. Обучение и своевременная дообучение персонала;

4. Создать единые политики безопасности, которые будут применяться ко всем системам виртуализации.

В малых и средних компаниях гетерогенная среда слишком затратна во всех смыслах, и является абсолютно нерентабельной. В крупных компаниях, зачастую, приходится иметь дело с подобной средой, и редко удаётся её избежать. Поэтому необходимо применить ряд мер, для грамотного расходования ресурсов, безопасности и эффективности управления. 
